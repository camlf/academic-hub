...
OMF_ENDPOINT = "http://localhost:5590/api/v1/tenants/default/namespaces/default/omf/"
API_KEY = "dummy"
...
omf_number_typeid, omf_number_type = omf_type("Double")
...
def omf_data(asset, name, timestamp, value):
    ...
    return {
        ...
        "values": [{"Timestamp": f"{ts.isoformat()}", "value": value}],
    }
...
def send_omf_message(
    ...
    if debug:
        data_preview = f"{str(data)[:120]}..." if message_type == "data" else data
        printg(
            f"\nDBG>>> {OMF_ENDPOINT[:24]}... {omf_headers(message_type, api_key, producer_token)} {data_preview}"
        )
        return
...
class OMFClient:
    @typechecked
    def __init__(self, api_key: str, printg=print, row_batch_size=100, debug=False):
        self._api_key = api_key
        self._init_ok = False
        self._row_batch_size = row_batch_size
        self._producer_token = "CSM_Test.EDS1"
    ...
    def create_streams(self, df, asset_name: str, printg=print, info_only=False, debug=False):
    ...
    def update_streams(
        self, df, printg=print, info_only=False, debug=False
    ):
